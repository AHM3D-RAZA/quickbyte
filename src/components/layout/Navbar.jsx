...
const navLinks = [
  { label: 'Home', path: '/' },
  { label: 'Browse Menu', path: '/restaurants' },
    ...
    <>
      ...
      <div className="w-full max-w-[1528px] mx-auto bg-brand-offwhite border-b border-x border-black/10 rounded-b-card overflow-hidden font-body text-black">
        <div className="h-auto lg:h-[70px] flex flex-col lg:flex-row lg:items-center lg:justify-between px-4 lg:px-[137px] py-3 lg:py-0 gap-2 lg:gap-0">
          ...
          {/* Right side: location + basket — collapses on mobile */}
          <div className="flex items-center justify-between lg:justify-end gap-4 lg:gap-9">
            <div className="hidden md:flex items-center gap-2 min-w-0">
              <img src={locationIcon} alt="Location Pin" className="w-[22px] h-[22px] lg:w-[25px] lg:h-[25px] object-contain shrink-0" />
            ...
            {/* Basket bar — full green pill on desktop, compact chip on mobile */}
            <div className="flex items-center bg-brand-green border border-black/10 rounded-br-card lg:rounded-br-card text-white font-body h-[50px] lg:h-full lg:w-[378px] ml-auto lg:ml-0">
              <div className="flex items-center justify-center px-2 lg:flex-1">
                <img src={basketIcon} alt="Basket" className="w-[30px] h-[30px] lg:w-[43px] lg:h-[43px] object-contain" />
              </div>
              ...
              <button className="hidden lg:flex flex-1 h-full items-center justify-center hover:bg-black/10 transition-colors cursor-pointer rounded-br-card">
                <img src={arrowDownIcon} alt="Go to checkout" className="w-[38px] h-[38px] object-contain rotate-90" />
              </button>
            </div>
      ...
      {/* ── Main nav row (logo + links + login) — desktop only ── */}
      <div className="w-full max-w-[1528px] mx-auto mt-6 lg:mt-[38px] hidden lg:block">
        <div className="px-6 flex items-center justify-between">
          <Link to="/">
            <img src={logo} alt="Order UK Logo" className="w-[180px] lg:w-[215px] h-auto object-contain" />
          </Link>
      ...
      {/* ── Mobile logo row + dropdown menu ── */}
      <div className="lg:hidden w-full px-4 pt-4">
        <Link to="/">
          <img src={logo} alt="Order UK Logo" className="w-[140px] h-auto object-contain" />
        </Link>
      </div>
      ...
      {menuOpen && (
        <div className="lg:hidden w-full px-4 py-4 flex flex-col gap-4 bg-white border-b border-black/10">
          ...
          <Link to="/login" onClick={() => setMenuOpen(false)} className="text-brand-orange font-semibold">
            Login/Signup
          </Link>
        </div>
      )}
